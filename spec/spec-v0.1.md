...
- **M0 — фундамент (без синка).** Сервер: схема, `/changes`, `/file` GET/PUT, токены. Плагин:
  настройки, ручная кнопка «скачать всё с сервера». Тест-харнесс, сценарии 1–3.
- **M1 — рабочий двусторонний синк.** Локальный индекс, дебаунс, очередь загрузок, конфликты по §5,
  шифрование v1. С этого момента можно осторожно пользоваться самому.
- **M2 — то, ради чего всё затевалось.** WebSocket push, rename, докачка, история ревизий в UI,
  восстановление удалённого.
- **M3 — продукт.** Шифрование путей, мультиволт, веб-морда для просмотра истории, установщик
  сервера одной командой, заявка в community plugins.

## 11. Открытые вопросы

- [ ] Мультиюзер или один волт на инстанс? (влияет на схему — решить до M0)
